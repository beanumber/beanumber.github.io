...
#let compact-header(title, description, location, date) = {
  __justify_align[
     #set text(
      size: 10pt,
      weight: "bold",
...
#let compact-entry(
  ...
  pad[
    #compact-header(title, description, location, date)
  ]
}
...
#let resume(
  ...
  show heading.where(level: 1): it => [
    ...
    #align(left)[
      #text[
        #strong[
          #text(color-accent)[#counter(heading).display()]
          #text(color-accent)[#it.body.text.slice(0, 3)]#text(color-darkgray)[#it.body.text.slice(3)]
        ]
      ]
